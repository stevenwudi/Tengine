                               ...
                               int letterbox_cols, int letterbox_rows)
{
    ...
    int score_jump = CLASS_NUM + 5;   // correspond to 4 coordinates and 1 objectiveness score.
    ...
    for (int h = 0; h <= feat_h - 1; h++)
    {
        for (int w = 0; w <= feat_w - 1; w++)
        {
            for (int a = 0; a <= anchor_num - 1; a++)
            {
                ...
                //process box score
                float box_score = feat[a * feat_w * feat_h * score_jump + (h * feat_w) * score_jump + w * score_jump +
                                       4];                float final_score = sigmoid(box_score ) * sigmoid(class_score);
                if (final_score >= prob_threshold)
                {
                    int loc_idx = a * feat_h * feat_w * score_jump + h * feat_w * score_jump + w * score_jump;
